...
// Порт отличается от чужого хелпера (47890), чтобы не конфликтовать,
// если на машине пользователя уже стоит сторонний клиент.
...
fn start(start_params: StartParams) -> impl Reply {
    // В релизной сборке — проверяем, что запускаемый файл это именно
    // наш mihomo.exe (по SHA256, зашитому при сборке хелпера), а не
    // произвольная программа. В debug-сборке проверку пропускаем,
    // чтобы удобно было тестировать с пересобираемым бинарником.
    if !cfg!(debug_assertions) {
        let sha256 = sha256_file(start_params.path.as_str()).unwrap_or_default();
        if sha256 != env!("TOKEN") {
            return format!(
                "SHA256 запускаемого файла: {}. Хелпер разрешает запускать только файл с хэшем: {}.",
                sha256,
                env!("TOKEN"),
            );
        }
